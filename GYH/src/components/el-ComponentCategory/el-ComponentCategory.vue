    ...
    export default {
        ...
        methods: {
            refresh:function(level){
                switch(level){
                    case 1:this.getFirstCats();break;
                    case 2:this.getSecondCats();break;
                    case 3:this.getThirdCats();break;
                }
            },
            getFirstCats:function(){
                var data = {};
                data.level = 1;
                $.post_json(appsettings.apiroot +(this.adminEdit===true?'admin/':'')+ "component/category/retrieve", data, this.callback_getFirstCats,data);
            },
            firstCatChanged: function (event) {
                this.selectedCats=[];
                var index=event.target.selectedOptions[0].index;
                if(index>0){
            ...
            getThirdCats: function () {
                ...
                $.post_json(appsettings.apiroot +(this.adminEdit===true?'admin/':'')+ "component/category/retrieve", data, this.callback_getThirdCats,data);
            },
                ...
                if (result != null && result.status == 0) {
                   ...
                   if(this.inited===true){
                    this.$emit('gotChildren',parentdata,this.thirdcats);
                        ...
                        if(this.selectedCats.length==3){
                            this.inited=true;
                            this.$emit('categoryChanged', this.selected_3rd_cat,this.selectedCats);
                        }
                    }
                } else {
            ...
            getSecondCats: function () {
                ...
                $.post_json(appsettings.apiroot +(this.adminEdit===true?'admin/':'')+ "component/category/retrieve", data, this.callback_getSecondCats,data);
            },
