												...
												<tr class="row-show" v-for="(pro,index) in projects" :key="index">
													...
													<td>
														<div class="btn-group op">
															<a class="btn btn-default btn-xs" target="_blank" :href="'/prj/edit.html?id='+pro.oid+'&c='+subjectCoreInfo.category_code+'&pid='+subject_oid+'&sid='+subject_oid+'&com='+pro.company_oid+'&t='+subject_type">修改</a>
															...
															<div class="dropdown-menu" style="width:500px;padding:8px 10px;background-color: #fff;">
																...
																<a href="javascript:void(0);" class="btn btn-xs btn-danger" @click="deleteProject(pro,index)">删除</a>
															</div>
														</div>
													</td>
...
<script>
	...
	export default {
		...
		methods: {
			...
			createNew: function () {
				window.open('/prj/edit.html?c=' + this.subjectCoreInfo.category_code + (this.subject_type == "p" ? "&pid=" :
						"&sid=") + this.subject_oid + "&t=" + this.subject_type + "&sn=" + this.subjectCoreInfo.subject_name +
					"&com=" + this.subjectCoreInfo.company_oid);
			},
