	...
	export default {
		...
		methods: {
			...
			getFavorites: function (page_index) {
				$.post_json(appsettings.apiroot + "account/favorite/retrieve", {
			...
			cancelFav: function (pro, index, t) {
				...
				pro.index = index;
				pro.type = t;
				var name = t == 'p' ? pro.product_name : (t == 's' ? pro.service_name : pro.company_name);
				//confirm
				this.$refs.pf.confirmShow({
					...
					success: function (res) {
						if (res.confirm == true) {
							...
							$.post_json(appsettings.apiroot + "account/favorite/cancel", data, that.callback_cancelFav, pro);
